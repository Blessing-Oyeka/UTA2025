...
void read_file(){
    ...
    TH1D *hRecoEnuInc_signal = new TH1D("hRecoEnuInc_signal","", nEnuBins, nEnuLow,nEnuHigh);//signal is selected true numu CC 
     ...
     TH1D *hResEnu_Vis_signal = new TH1D("hResEnu_Vis.signal", "", nEnuBins, -1, 1);
     TH1D *hResEnu_t2kqe_signal = new TH1D("hResEnu_Vis.signal", "", nEnuBins, -1, 1);
	...
	if ( true_topology == 2||true_topology == 3 || true_topology == 4 ){
	  hRecoEnuInc_signal->Fill(getEnuInc(vmomprt, vmompi, dmommu));
	  hRecoEnuT2k_IncSignal->Fill(getEnuT2k(dmommu, costheta_mu, dEb));	  
	}
      }
