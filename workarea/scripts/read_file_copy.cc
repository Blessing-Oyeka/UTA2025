...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
double getEnuArgoneut(std::vector<double> &Tprt, std::vector<double> &momprtT, double mommu, double cosTheta_mu, double MA_minus_2 = 37, double Emiss = 0.03) {

  double val = 0;
  val += sqrt(pow(mommu, 2) + pow(mmuon, 2));
  for (unsigned int ii = 0; ii < Tprt.size(); ++ii)
    val += Tprt[ii];

  ////// Reconstructing muon transverse momentum using sin(theta)
  double sinTheta_mu = sqrt(1.0 - pow(cosTheta_mu, 2));
  double mommuT = mommu * sinTheta_mu;

  //////////// Computing the total transverse momentum of all protons
  double totalProtonMomT = 0.0;
  for (unsigned int ii = 0; ii < momprtT.size(); ++ii)totalProtonMomT += momprtT[ii]; 

  ////////////////// missing transverse momentum (scalar approx)
  double mommissT = fabs(mommuT - totalProtonMomT);

  ///////////////////////////////////// transverse correction term
  val += pow(mommissT, 2) / (2.0 * MA_minus_2);
...
void read_file_copy(){
    ...
    TH1D *hRecoEnuInc_Np0pi = new TH1D("hRecoEnuInc_Np0pi","",nEnuBins, nEnuLow, nEnuHigh);////////////////////NEW HIST
    TH1D *hResEnuInc_Np0pi_S = new TH1D("hResEnuInc_Np0pi_S","",nEnuBins, -1, 1);////////////////////NEW HIST
    ...
    TH1D *hResEnuNp0pi_argo = new TH1D("hResEnuNp0pi_argo", "", nEnuBins, -1, 1);///////////////argo res 1D
    TH1D *hResEnuNp0pi_argo_sig = new TH1D("hResEnuNp0pi_argo_sig", "", nEnuBins, -1, 1);///////////////argo res 1Dsignal

    TH1D *hRecoEnuNp0pi_argo = new TH1D("hRecoEnuNp0pi_argo", "", nEnuBins, nEnuLow, nEnuHigh);////////////////ARGONEUT RECO  1D
    TH1D *hRecoEnuNp0pi_argo_sig = new TH1D("hRecoEnuNp0pi_argo_sig", "", nEnuBins, nEnuLow, nEnuHigh);////////////////ARGONEUT RECO SIGNAL 1D

    TH1D *hTrueEnu_Np0pi = new TH1D("hTrueEnu_Np0pi","",50, 0,3);
    ...
    TH2D *hResEnuNp0pi_argo_sig2D = new TH2D("hResEnuNp0pi_argo_sig2D", "", nEnuBins, nEnuLow, 7.0, nEnuBins,-1, 1);///////////////ARGONEUT RES SIGNAL 2D
    TH2D *hResEnuInc_Np0pi_S2D = new TH2D("hResEnuInc_Np0pi_S2D", "", nEnuBins, nEnuLow, 7.0, nEnuBins,-1, 1);////////////////////NEW HIST
    ...
    std::vector<double>vRecoEnuNp0pi_argo_sig;
    std::vector<double>vResEnuNp0pi_argo_sig;
    std::vector<double>vResEnuInc_Np0pi_S;
    ...
    //loop over slices
     ...
     icarus_tree->GetEntry(slc);
      ...
      std::vector<double> vTprt;
      std::vector<double> vmomprtT;
      ...
      for(int trkid : slice_track_index[slc]){
	    ...
	    if (reco_track_pdg == 2212) {
	      n_protons++;
	      double reco_track_sinT = sqrt(1 - pow(reco_track_cosT, 2));
	      vmomprtT.push_back(reco_track_momenta * reco_track_sinT); // transverse momentum
	      vTprt.push_back(pow(reco_track_momenta, 2) / (2 * mprt)); // kinetic energy
	    }
	...
	if (
	    n_protons >= 1 &&
	    true_topology == 3 &&
	    true_neutrino_flavour == 14 &&
	    true_neutrino_current_type == 1
	    ) {
	  double EnuArgo = getEnuArgoneut(vTprt, vmomprtT, dmommu, costheta_mu);
	  hRecoEnuNp0pi_argo->Fill(EnuArgo);
	  hResEnuNp0pi_argo->Fill((true_energy_neutrino - EnuArgo) / true_energy_neutrino);
	  double EnuInc = getEnuInc(vmomprt, vmompi, dmommu);
	  hRecoEnuInc_Np0pi->Fill(EnuInc);
	  hTrueEnu_Np0pi->Fill(true_energy_neutrino);
	  hResEnuInc_Np0pi_S->Fill((true_energy_neutrino - EnuInc) / true_energy_neutrino);
	  hResEnuInc_Np0pi_S2D->Fill(EnuInc, (true_energy_neutrino - EnuInc) / true_energy_neutrino);/////////////////////////////fill this
	  hRecoEnuNp0pi_argo_sig->Fill(EnuArgo);
	  hResEnuNp0pi_argo_sig->Fill((true_energy_neutrino - EnuArgo) / true_energy_neutrino);
	  hResEnuNp0pi_argo_sig2D->Fill(EnuArgo, (true_energy_neutrino - EnuArgo) / true_energy_neutrino);
	  vRecoEnuNp0pi_argo_sig.push_back(EnuArgo);
	  vResEnuNp0pi_argo_sig.push_back((true_energy_neutrino - EnuArgo) / true_energy_neutrino);
	  vResEnuInc_Np0pi_S.push_back((true_energy_neutrino - getEnuInc(vmomprt, vmompi, dmommu))/true_energy_neutrino);
	}
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	/*if (n_protons >= 1 && (true_topology == 3)) {
	  double EnuArgo = getEnuArgoneut(vTprt, vmomprtT, dmommu, costheta_mu);
	  hRecoEnuNp0pi_argo->Fill(EnuArgo);
	  hResEnuNp0pi_argo->Fill((true_energy_neutrino - EnuArgo) / true_energy_neutrino);
	  hRecoEnuInc_Np0pi->Fill(getEnuInc(vmomprt, vmompi, dmommu));
	  hTrueEnu_Np0pi->Fill(true_energy_neutrino);
	  hResEnuInc_Np0pi_S->Fill((true_energy_neutrino - getEnuInc(vmomprt, vmompi, dmommu))/true_energy_neutrino);
	  hResEnuInc_Np0pi_S2D->Fill(getEnuInc(vmomprt, vmompi, dmommu), (true_energy_neutrino - getEnuInc(vmomprt, vmompi, dmommu))/true_energy_neutrino);
	  

	  if (true_neutrino_flavour == 14 && true_neutrino_current_type == 1) { // signal: numu CC
	    hRecoEnuNp0pi_argo_sig->Fill(EnuArgo);
	    hResEnuNp0pi_argo_sig->Fill((true_energy_neutrino - EnuArgo) / true_energy_neutrino);
	    hResEnuNp0pi_argo_sig2D->Fill(EnuArgo, (true_energy_neutrino - EnuArgo) / true_energy_neutrino);
	...
	if ( true_topology == 4 ){
     ...
     //output file, unique and doesnt require repitition
      ...
      TVectorD vRecoEnuNp0pi_argo_sig_T(vRecoEnuNp0pi_argo_sig.size());
      for (int i = 0; i<vRecoEnuNp0pi_argo_sig.size();++i)vRecoEnuNp0pi_argo_sig_T[i]=vRecoEnuNp0pi_argo_sig[i];
      vRecoEnuNp0pi_argo_sig_T.Write("vRecoEnuNp0pi_argo_sig_T");

      TVectorD vResEnuNp0pi_argo_sig_T(vResEnuNp0pi_argo_sig.size());
      for (int i = 0; i<vResEnuNp0pi_argo_sig.size();++i)vResEnuNp0pi_argo_sig_T[i]=vResEnuNp0pi_argo_sig[i];
      vResEnuNp0pi_argo_sig_T.Write("vResEnuNp0pi_argo_sig_T");

      TVectorD vResEnuInc_Np0pi_S_T(vResEnuInc_Np0pi_S.size());
      for (int i = 0; i<vResEnuInc_Np0pi_S.size();++i)vResEnuInc_Np0pi_S_T[i]=vResEnuInc_Np0pi_S[i];
      vResEnuInc_Np0pi_S_T.Write("vResEnuInc_Np0pi_S_T");
      ...
      hResEnuNp0pi_argo->Write();
      hResEnuNp0pi_argo_sig->Write();
      hRecoEnuNp0pi_argo->Write();
      hRecoEnuNp0pi_argo_sig->Write();
      hResEnuNp0pi_argo_sig2D->Write();
      hRecoEnuInc_Np0pi->Write();

      hTrueEnu_Np0pi->Write();
      hResEnuInc_Np0pi_S->Write();

      hResEnuInc_Np0pi_S2D->Write();
